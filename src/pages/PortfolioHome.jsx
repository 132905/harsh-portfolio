...
import { Github, Linkedin, Mail, Download, Sun, Moon, X } from "lucide-react";
...
export default function PortfolioHome() {
  ...
  const skills = [
    ...
    {
      title: "Libraries",
      items: ["TensorFlow", "Scikit-learn", "NumPy", "Pandas", "PyTorch", "OpenCV"],
    },
    {
      title: "ML / AI",
      items: ["CNN", "NLP", "Generative AI", "Classification", "Clustering"],
    },
    {
      title: "Tools",
      items: ["VS Code", "Git", "Docker", "Jupyter Notebook"],
    },
    {
  ...
  return (
    <main className="bg-white text-black dark:bg-black dark:text-white transition-colors duration-300 font-sans">
      ...
      <section className="flex flex-col items-center justify-center text-center py-20 px-4">
        ...
        <motion.p
          className="text-xl md:text-2xl text-red-500 dark:text-red-400"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          ...
          {skills.map(({ title, items }, i) => (
            <div key={i} className="bg-gray-100 dark:bg-gray-900 rounded-xl p-4 shadow">
              ...
              <ul className="flex flex-wrap gap-2 text-sm text-gray-800 dark:text-gray-300">
                {items.map((skill, idx) => (
                  <li key={idx} className="bg-gray-200 dark:bg-gray-800 px-3 py-1 rounded-full">
                    {skill}
                  </li>
                ))}
              </ul>
      ...
      <AnimatePresence>
        {modalData && (
          ...
          >
            <motion.div
              className="bg-white dark:bg-gray-900 text-black dark:text-white p-6 rounded-xl max-w-lg w-full relative"
              initial={{ scale: 0.8 }}
              animate={{ scale: 1 }}
              exit={{ scale: 0.8 }}
            >
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">
                <strong>Tech Stack:</strong> {modalData.tech}
              </p>
              <p className="mb-2">
